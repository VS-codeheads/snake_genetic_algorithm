...
Existing literature demonstrates that CNNs excel in high-dimensional visual domains where hand-crafting features is infeasible~\cite{mnih2015human,krizhevsky2012imagenet}. However, for games with clear, interpretable structure, engineered features often outperform raw inputs, particularly under constrained training budgets~\cite{botvinick2020deep}. Our work contributes empirical evidence to this ongoing debate by systematically comparing both approaches in a controlled environment.
...
The feature state vector consists of 10 carefully designed dimensions capturing task-relevant information:
\begin{enumerate}
\item Relative food X-position (normalized, $\Delta x \in [-2, 2]$)
\item Relative food Y-position (normalized, $\Delta y \in [-2, 2]$)
\item Collision threat left (binary)
\item Collision threat front (binary)
\item Collision threat right (binary)
\item One-hot direction encoding: up (binary)
\item One-hot direction encoding: down (binary)
\item One-hot direction encoding: left (binary)
\item One-hot direction encoding: right (binary)
\item Normalized snake body length
\end{enumerate}

These features encode minimum information necessary for optimal play: food location, immediate danger detection, current orientation, and progress tracking~\cite{sharma2017autonomoussnake}. The vector is processed by a feed-forward network with 2 hidden layers (128 units, ReLU), totaling $\sim$18K parameters.
...
This representation forces the agent to learn spatial patterns from raw grid data~\cite{mnih2015human}. The CNN architecture processes this input through three convolutional layers with progressively increasing feature maps:

\textbf{Layer sequence:} Conv(3→32, k=5, p=2)→ReLU→MaxPool(2) [30→15]; Conv(32→64, k=3, p=1)→ReLU→MaxPool(2) [15→7]; Conv(64→128, k=3, p=1)→ReLU; Flatten→FC(256)→ReLU→Dropout(0.2)→FC(4)~\cite{krizhevsky2012imagenet}

The first layer captures local spatial patterns with $5 \times 5$ kernels. Subsequent $3 \times 3$ kernels enable hierarchical feature extraction. Max-pooling reduces spatial dimensions while preserving features. Final fully connected layers aggregate spatial information. Total: $\sim$320K parameters, 18× larger than feature-based network.

\subsection{Training Protocol}

Both agents used Deep Q-Learning with experience replay and target networks~\cite{lin1993reinforcement,watkins1992q}. Hyperparameters were carefully selected based on established DRL best practices~\cite{sutton2018reinforcement}:
...
\begin{itemize}
    ...
    \item \textbf{Exploration}: $\epsilon$-greedy with linear decay from $1.0$ to $0.1$ over 5,000 episodes~\cite{francois2015introduction}
\end{itemize}
...
Mnih, V., Kavukcuoglu, K., Silver, D., Graves, A., Antonoglou, I., Wierstra, D., \& Riedmüller, M. (2013).
...
\newblock \emph{In AAAI} (Vol. 16, pp. 2094--2100).

\bibitem{krizhevsky2012imagenet}
Krizhevsky, A., Sutskever, I., \& Hinton, G. E. (2012).
\newblock Imagenet classification with deep convolutional neural networks.
\newblock \emph{In Advances in neural information processing systems} (pp. 1097--1105).

\bibitem{sharma2017autonomoussnake}
Sharma, S., Sharma, M., \& Atreja, G. (2017).
\newblock Autonomous snake game using deep Q-learning.
\newblock \emph{arXiv preprint arXiv:1712.07124}.
...
\bibitem{francois2015introduction}
Françoise-Lavet, V., Henderson, P., Las Casas, D., Fonteneau, R., \& Ernst, D. (2018).
\newblock Introduction to deep reinforcement learning.
\newblock \emph{Foundations and Trends in Machine Learning}, 11(3-4), 219--354.

\bibitem{botvinick2020deep}
Botvinick, M., Wang, J. X., Kumaran, D., Barto, A., \& Leibo, J. Z. (2020).
\newblock Reinforcement learning, fast and slow.
\newblock \emph{Trends in Cognitive Sciences}, 23(5), 408--422.
